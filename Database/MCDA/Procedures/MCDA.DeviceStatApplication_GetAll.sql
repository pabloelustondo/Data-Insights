...
BEGIN
	...
	BEGIN
		...
		ELSE IF (@status = 0)	--Success
			BEGIN
				...
				from (
					select top (@BatchSize) [timestamp] as ts from [$(MobiControlDB)].dbo.DeviceStatApplication WITH (NOLOCK) 
	...
	BEGIN
		Select @lastTime = max(a.ts) from (
			select top (@BatchSize) [timestamp] as ts from [$(MobiControlDB)].dbo.DeviceStatApplication WITH (NOLOCK) order by [timestamp] asc
			) a
